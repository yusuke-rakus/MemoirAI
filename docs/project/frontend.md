...
- Settings Dialogはプロフィール、一般、メモリ、共有した日記、アカウントの5 tabです。共有した日記では公開中のコピーを一覧し、共有解除できます。account削除はGoogle再認証後にclient-side gatewayでapp dataを削除し、契約同意記録へ5年TTLを設定してからAuth accountを削除します。
