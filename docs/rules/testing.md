...
- 自動テストは、UIに依存しない機能上の契約を保証する。データ変換、認証・権限、永続化、外部サービス呼び出し、エラー処理、二重実行防止、安全性を対象にする。
- UIの表示・操作・画面遷移を検証する自動テストは作成しない。React Testing Libraryでのcomponent描画、DOM assertion、クリック・キーボード操作、アクセシビリティ属性、loading / empty / error表示の検証は対象外とする。
- UI componentに機能上重要なロジックがある場合は、UIから純粋関数・hook・serviceへ抽出し、そのUI非依存の契約だけをテストする。
- Markdownの変換結果や危険なHTMLの除去など、DOMを描画しなければ検証できない安全性も自動テストの対象外とする。入力値と変換結果を扱うUI非依存の実装へ分離できる場合だけ、その実装をテストする。
- レスポンシブ、テーマ、余白、配置、装飾、motion、文言、画面上の改行、UI操作は、影響するUI変更時の手動確認として扱う。jsdomのDOM assertionを実ブラウザでの表示・操作確認として報告しない。
- テスト名には条件と機能結果を記述する。目的を失ったケースやassertionのないケースは残さない。
