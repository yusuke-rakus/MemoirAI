...
export type PreparedDiaryImage = {
  blob: Blob;
  width: number;
  height: number;
  contentType: string;
...
type CompressionAttempt = { blob: Blob | null; minimumQualitySize: number };
...
const findBestCompression = async (
  ...
  if (minimumQualityBlob.size > DIARY_IMAGE_TARGET_MAX_BYTES) {
    return { blob: null, minimumQualitySize: minimumQualityBlob.size };
  }
  ...
  return { blob: bestBlob, minimumQualitySize: minimumQualityBlob.size };
};
...
export const prepareDiaryImage = async (
  file: File,
): Promise<PreparedDiaryImage> => {
